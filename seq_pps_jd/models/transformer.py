...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, iter, query, inputs, mask, add_matrix):
        if (iter != 0):
        ...
        context = self.self_attn(input_norm, input_norm, input_norm, add_matrix,mask=mask)
...
class TransformerEncoder(nn.Module):
    ...
    def encode(self, input_vecs, mask, add_matrix, use_pos=True):
        ...
        for i in range(self.num_inter_layers):
            x = self.transformer_inter[i](i, x, x, 1 - mask, add_matrix)  # all_sents * max_tokens * dim

        x = self.layer_norm(x)
        #out_pos can be 0 or -1 # represent query or item in the item_transformer model
        return x

    def forward(self, input_vecs, mask, add_matrix, use_pos=True, out_pos=0):
        """ See :obj:`EncoderBase.forward()`"""

        #out_pos can be 0 or -1 # represent query or item in the item_transformer model
        x = self.encode(input_vecs, mask, add_matrix, use_pos)
